...
(payroll_cuts2 <- c(0, 2e6, 5e6, 10e6, Inf, legal_cuts) |> unique() |> sort())
paygroup_labels2 <- cutlabs(payroll_cuts2)
...
mta3 <- mta2 |> 
  ...
  summarise(taxnow = sum(tax),
            tax1bp = sum(tax1bp),
            .by=c(taxability, area, owner, paygroup, paylabel)) |> 
  arrange(taxability, paygroup, paylabel, area) 
