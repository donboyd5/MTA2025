...
get_tax <- function(data, taxplan){
  df <- data |>
    ...
    select(-lower, -upper) |> 
    mutate(tax = ifelse(exempt | excluded | low_pay,
                        0,
                        rate * gpayroll))
...
taxcomp <- bind_rows(
  pmtfc |> 
    mutate(get_tax(pick(everything()), prob$plan1)),
  pmtfc |> 
    mutate(get_tax(pick(everything()), prob$plan2)))

glimpse(taxcomp)
count(taxcomp, shortname)
taxcomp |> 
  summarise(gpayroll=sum(gpayroll), .by=shortname)
