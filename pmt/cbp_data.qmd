...
tmp2 <- shares |> 
  filter(fipscty=="047", naics=="484121") |>  # get a single group of rows
  mutate(res=ests_env$get_estabs_avgemp(est=est[1], emp=emp[1], estknown=known, 
                                        lb_avgemp=lb, ub_avgemp=ub,
                                        threshold=.5, trace=TRUE) |> list())

estemp <- shares |> 
  filter(fipscty=="047", naics=="484121") |>
  nest(data = -c(fipscty, state, county, naics, description)) |>
  mutate(res = purrr::map(data, ~ests_env$get_estabs_avgemp(
    est = .x$est[1],
    emp = .x$emp[1],
    estknown = .x$known,
    lb_avgemp = .x$lb,
...
estemp |> 
  mutate(
    estabs = purrr::map(res, "estabs"),
    avgemp = purrr::map(res, "avgemp")
  ) |>
  unnest_longer(c(estabs, avgemp)) |>
  unnest(data)
...
ee2 <- estemp |> 
  mutate(obj=pluck(res, 1, "value"),
         par=pluck(res, 1, "par") |> list())
...
shares |> 
  filter(fipscty=="047", nmiss > 0) |>
  filter(naics %in% keepnaics) |>
  select(-c(avgemp1:avgdiff1)) |> 
  nest(data = c(name, known, lb, ub)) |> 
  head(1) |> 
  mutate(res = purrr::map(data, \(x) ests_env$get_estabs_avgemp(
    est = x$est[1],
    emp = x$emp[1],
...
estemp <-shares |> 
  # filter(fipscty=="047", naics=="484121") |> # good
  # filter(fipscty=="047", naics %in% keepnaics) |> # good
  filter(mta) |> 
  nest(data = -c(fipscty, state, county, naics, description)) |>
  # filter(row_number() <= 30) |> # good 3, 10, 30
