...
tmp <- matched |> filter(is.na(payrolladj))
...
# synrecsx <- function(nrecs, lb, ub, firms, emp, payroll_grp){
#   # created weighted records where each rec represents a set of firms
#   # return empty tibble if invalid input
#   if(nrecs == 1 || lb > ub || is.na(lb) || is.na(ub)) return(tibble(nemp = integer(0)))
#   
...
check <- matched2 |> 
  filter(owner=="Private", area=="Bronx County", naics=="23") |> 
  mutate(nrecs = case_when(as.integer(fscode) %in% 2:18 ~ 20,
                           fscode == "99" ~ 20,
                           .default = 1)) |> 
  mutate(data = pmap(
    list(nrecs, fslb, fsub, sfirms, semp, qpayroll_tot),
    synrecs))
