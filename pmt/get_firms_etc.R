...
obj <- function(params, n, empest, threshold, ...) {
  firms <- params[1:n]
...
heq <- function(params, n, firmsum, empsum, ...) {
  ...
  # each must equal 0
  return(c(
    sum(firms) - firmsum,        # equality: sum(estabs) = est
    sum(firmsum * avgemp) - empsum  # equality: sum(estabs * avgemp) = emp
  ))
}
...
df <- data |>
  select(fslb, fsub, firms, emp, payroll, firms_grp, emp_grp, payroll_grp, maxemp, excess)
df
