...
df |> 
  summarise(estabs=first(estabs),
            ...
            emp=first(emp),
            emp_sum=sum(esize_estabsadj * esize_emp),
            payroll=first(payroll),
            payroll_sum=sum(esize_estabsadj * esize_emp * payroll / emp),
            paypest = payroll_sum / esize_) |> 
  mutate(estpd=estabs_sum / estabs - 1,
         emppd=emp_sum / emp - 1,
         paypd=payroll_sum / payroll -1)
...
# df <- d |> 
#   mutate(fempest(emp, estabs, elb, eub, esize_estabs)) |> 
#   mutate(esize_payroll=payroll * esize_estabsadj * esize_avgemp / sum( esize_estabsadj),
#          esize_epest = esize_payroll / esize_estabsadj)

count(qclong, status)

# Dutchess 115, 517, 711
# Kings 212, 516, 561, 
# Nassau 316

d <- qclong |> 
  filter(owner=="Private", status== "tbd") |> 
  filter(area=="Dutchess", naics=="711") # |>
  # filter(area=="Dutchess", naics=="115") |> 
d
# dput(sv)
...
df2 <- df |> 
  mutate(esize_emp = esize_avgemp * esize_estabsadj,
         esize_payroll = (payroll / emp) * esize_emp,
         payroll_pest = esize_payroll / esize_estabsadj)

summary(df2)
quantile(df2$payroll_pest, na.rm=TRUE)
skimr::skim_without_charts(df2)

df2 |> 
  summarise(esize_payroll=sum(esize_payroll), .by=area) |> 
  arrange(area)

df2 |> 
  summarise(payroll=sum(first(payroll)), .by=c(area, naics)) |> 
  summarise(payroll=sum(payroll), .by=area)

tmp <- count(df, naics, qtitle)
df2 |> 
  filter(naics=="236", area=="Bronx") |> 
  select(-c(year, naicslev, status, rematch, replaced, converge))

df
count(df, converge)
df |> filter(area=="Putnam")
df |> filter(converge==1)

# 35 of 858 groups did not converge properly
# 8 hit max iter
# 27 had linesearch error
